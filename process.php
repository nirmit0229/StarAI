...
session_start();
	...
	if(isset($_POST['begin'])){
		$person_id = (rand(1000,10000));
		$birth_date= $_POST['bdate'];
		$birth_time= $_POST['btime'];
		$timezone= $_POST['ztime'];
		$place_birth= $_POST['pbirth'];
		$sex= $_POST['sex'];
		$email= $_POST['email'];
		$record_created = (date("Y-m-d"));

			$sql = "INSERT INTO `person` (`person_id`, `birth_date`, `birth_time`, `place_of_birth`,`sex`,`timezone`,`email`,`record_created`) VALUES ('$person_id','$birth_date','$birth_time','$place_birth','$sex',
			    '$timezone','$email','$record_created')";
